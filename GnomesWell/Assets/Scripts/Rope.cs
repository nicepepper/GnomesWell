...
public class Rope : MonoBehaviour
{
    ...
    // Объект твердого тела, к которому следует присоеденить конец веревки.
    ...
    void CreateRopeSegment()
    {
        ...
        {
            Debug.LogError("Rope segment body prebaf has no" + "Rigidbody2D and/or SpringJoint2D1");
            return;
        }
    ...
    private void Update()
    {
        ...
        if (isDecreasing)
        {
            // Веревку нужно укоротить. Если длина сочленения
        ...
        if (lineRenderer != null)
        {
            ...
            // Последняя точка соответствует точке привязк
            // несущего объекта.
            SpringJoint2D connectedObjectJoint = connectedObject.GetComponent<SpringJoint2D>();
